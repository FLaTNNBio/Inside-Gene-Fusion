...
def main():
    ...
    num_total = len(dataset_with_bert)
    num_train = int(train_ratio * num_total)
    num_val = int(val_ratio * num_total)
    num_test = num_total - num_train - num_val
    
    # Split the dataset into train, validation, and test sets
    train_dataset, val_dataset, test_dataset = random_split(dataset_with_bert, [num_train, num_val, num_test])
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        val_loss, val_accuracy = evaluate(model, device,val_loader,loss_fn)
