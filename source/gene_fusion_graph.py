...
# Training function
def train(train_loader,model,device,optimizer,loss_fn):
...
def main():
    ...
    graph_data_list = torch.load("../graph_data_1.pth")
    ...
    # Extract labels (assuming 'graph_data_list' is a list of data objects with attribute 'y')
    labels = [data.y.item() for data in graph_data_list]  # Assuming each graph has one label
    
    # First, split the dataset into train+val and test with stratification
    train_val_data, test_data, train_val_labels, test_labels = train_test_split(
        graph_data_list, labels, test_size=0.1, stratify=labels, random_state=42)
    
    # Then, split the train+val set into training and validation sets with stratification
    train_data, val_data, train_labels, val_labels = train_test_split(
        train_val_data, train_val_labels, test_size=0.1111, stratify=train_val_labels, random_state=42)
    ...
    for epoch in range(1, num_epochs + 1):
        train_loss = train(train_loader,model,device,optimizer,loss_fn)
    ...
    test_roc_auc, test_preds, test_labels = compute_roc_auc(test_loader,model,device)
